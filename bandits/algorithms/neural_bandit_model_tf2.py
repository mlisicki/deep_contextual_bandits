...
class NeuralBanditModel(BayesianNN):
  ...
  def build_layer(self, x, num_units):
    ...
    nn = tf.keras.layers.LayerNormalization(name=None,axis=-1,epsilon=1e-12,dtype=tf.float32)(nn)

    if dropout:
      nn = tf.nn.dropout(nn, 1 - (1 - (1 - (self.hparams.keep_prob))))
    ...
    with self.graph.as_default():
      ...
      with tf.compat.v1.name_scope(self.name):
        ...
        self.summary_writer = tf.compat.v1.summary.FileWriter(
            "{}/graph_{}".format(LOGDIR, self.name), self.sess.graph)
